...
Common failure modes for the social-content-engine skill.

---

## Gotcha: Rebuilding the calendar instead of extending it

**Why it matters:** The calendar is the durable artifact — it is the reason
this is a content operation and not a campaign generator. When each run starts
from a blank calendar, the owner re-approves dates they already approved, and
posts that were scheduled last month quietly get duplicated or dropped.

### ✗ Bad

```
Owner: "Give me another month of posts."
→ Build a fresh 30-day calendar from scratch.
→ Two rows collide with posts already scheduled in HubSpot.
```

### ✓ Good

```
Open the existing calendar first, then report:
  Published since last run: 6 posts (Jun 2 – Jun 24)
  Still scheduled:          4 posts (through Jul 6)
  Gap starting:             Jul 7

"I'll extend from Jul 7. Same cadence, three a week?"
```

---

## Gotcha: Writing a caption before reading the voice profile

**Why it matters:** Voice fidelity is the stated adoption barrier for this
skill. Owners do not abandon it because the graphics are wrong; they abandon
it because the caption sounds like a brand account and not like them. The
shared profile exists so a correction made once holds across every skill.

### ✗ Bad

Draft 12 captions in the owner's general industry register, present them, and
rewrite all 12 after the owner says "this doesn't sound like me."

### ✓ Good

Read `../shared/voice-profile.md` first. Anchor on the never-dos before
each caption, not just the first. If no profile exists, build one from their
own posts and confirm it before drafting at volume.

---

## Gotcha: Restating a Shopify price from memory

**Why it matters:** A wrong price in a public post cannot be quietly fixed
after a hundred people have seen it. Prices change between sessions, and the
calendar is long-lived, so a number carried over from an earlier run is
routinely stale.

### ✗ Bad

"The mini-split was USD 2,400 last time we posted — I'll use that again."

### ✓ Good

Read `variants[].price` fresh for every post that names a number. If the
connector did not return one, say the price is unavailable and ask. Only claim
"was X, now Y" when `compare_at_price` exists and is genuinely higher.
...
→ Step 6 generates 6 social designs + 2 email designs.
...
```
Step 2: Calendar built with Path column.
  - 6 rows tagged Canva (social)
  - 2 rows tagged Text-only (no Canva)
Step 6: Generates exactly 6 Canva designs. Email rows are not passed
        to any Canva endpoint.
Step 8: Drafts 6 social captions + 2 full emails (subject + preheader +
        body) as plain text.
Step 9: Stages 6 social posts. Surfaces 2 emails inline for the owner
        to copy into their email tool.
```
...
```
Step 4: "8 Canva rows × 3 candidates = 24 designs, ~120 API calls.
Proceed at default 3 candidates per row?"

Step 6: row 1's 3 candidates fire in parallel → 30s pause → row 2 →
...
(Note: email rows skip Canva entirely in this skill — see SKILL.md Step 2.
...
When you detect the broken design during Step 6 verification, regenerate just
